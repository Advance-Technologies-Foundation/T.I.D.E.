...
namespace AtfTIDE.QueryExecutor {
	...
	public class EsqFilterParser : IEsqFilterParser {
		...
		private static IEnumerable<string> GetFolderFilterStringCollectionValueByKey(
			IEnumerable<EntitySchemaQueryFilter> entitySchemaQueryFilters, string key){
			IEnumerable<EntitySchemaQueryExpressionCollection> rightExpressions =
				entitySchemaQueryFilters
					.Where(f => f.IsEnabled && f.LeftExpression.Path == key)
					.Select(s => s.RightExpressions);

			List<string> collection = new List<string>();
			foreach (EntitySchemaQueryExpressionCollection rightExpression in rightExpressions) {
		...
		public string GetEsqFilterValueByKey(string key, EntitySchemaQuery esq){
			...
			IEnumerable<string> ffKeys = filterGroups.Keys.Where(k => k.StartsWith("folderFilter"));
		...
		public string GetSearchBoxFilterValue(EntitySchemaQuery esq){
			...
			if (!sbKey.IsNullOrWhiteSpace() &&
				...
				IEnumerable<EntitySchemaQueryFilter> entitySchemaQueryFilters
					= searchBoxFilter as EntitySchemaQueryFilter[] ?? searchBoxFilter.ToArray();
				sbValue
					= entitySchemaQueryFilters.FirstOrDefault()?.RightExpressions?.FirstOrDefault()?.ParameterValue
											.ToString() ??
					string.Empty;
			}
			return sbValue;
